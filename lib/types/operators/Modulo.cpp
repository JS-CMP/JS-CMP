...
JS::Any JS::Any::operator%(const JS::Any& other) const {
    switch (this->value.index()) {
        ...
        case STRING:
            switch (other.value.index()) {
                case NUMBER:
                    return JS::Any(std::fmod(std::stod(std::get<Rope>(this->value).toString()), std::get<double>(other.value)));
                case STRING:
        ...
        case BOOL:
            switch (other.value.index()) {
                case NUMBER:
                    return JS::Any(std::fmod(static_cast<double>(std::get<bool>(this->value)), std::get<double>(other.value)));
                case STRING:
...
JS::Any JS::Any::operator%(JS::Undefined) const {
    switch (this->value.index()) {
        ...
        case STRING:
            return JS::Any(std::fmod(std::stod(std::get<Rope>(this->value).toString()), std::numeric_limits<double>::quiet_NaN()));
        case BOOL:
            return JS::Any(std::fmod(static_cast<double>(std::get<bool>(this->value)), std::numeric_limits<double>::quiet_NaN()));
        default:
            return JS::Any(std::numeric_limits<double>::quiet_NaN());
    }
}
...
namespace JS {
    ...
    Any operator%(JS::Undefined value, JS::Any const& any) {
        switch (any.value.index()) {
            ...
            case STRING:
                return JS::Any(std::fmod(std::numeric_limits<double>::quiet_NaN(), std::stod(std::get<Rope>(any.value).toString())));
            case BOOL:
                return JS::Any(std::fmod(std::numeric_limits<double>::quiet_NaN(), static_cast<double>(std::get<bool>(any.value))));
            default:
                return JS::Any(std::numeric_limits<double>::quiet_NaN());
