        ...
        switch (this->value.index()) {
            case NUMBER:
                ...
                switch (other.value.index()) {
                    ...
                    case NULL_TYPE:
                        return JS::Any(std::get<double>(
                            this->value)); // Number - Null is Number
                    default:
                        return JS::Any(
            ...
            case STRING:
                ...
                switch (other.value.index()) {
                    ...
                    case NULL_TYPE:
                        return JS::Any(std::stod(
                            std::get<Rope>(other.value)
                                .toString())); // String - Null is treated as
            ...
            case BOOL:
                ...
                switch (other.value.index()) {
                    ...
                    case NULL_TYPE:
                        return JS::Any(
                            std::get<bool>(this->value)); // Boolean - Null is
                                                          // treated as Boolean
                    default:
                        return JS::Any(
                            std::numeric_limits<double>::quiet_NaN());
            ...
            case NULL_TYPE:
                ...
                switch (other.value.index()) {
                    case NUMBER:
                        return JS::Any(-std::get<double>(
                            other.value)); // Null - Number is -Number
                    case STRING:
                        return JS::Any(-std::stod(
                            std::get<Rope>(other.value)
                                .toString())); // Null - String is -String
                    case BOOL:
                        return JS::Any(-static_cast<double>(std::get<bool>(
                            other.value))); // Null - Boolean is -Boolean
                    case NULL_TYPE:
            ...
            default:
                return JS::Any(
                    std::numeric_limits<double>::quiet_NaN()); // Invalid type
        }
    } catch (const std::invalid_argument&) {
        return JS::Any(
            std::numeric_limits<double>::quiet_NaN()); // Handle conversion
                                                       // errors
    }
}