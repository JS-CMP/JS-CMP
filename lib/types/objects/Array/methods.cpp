...
bool Array::defineOwnProperty(const std::u16string& key, JS::Attribute attribute, bool is_throw) {
    ...
    if (JS::IS::ArrayIndex(key)) {
        ...
        if (!succeeded) {
            if (is_throw) {
                throw std::runtime_error("Cannot define property: " + JS::CONVERT::ToString(key)); // TODO: TypeError
            }
            return false;
        }
        if (index >= oldLen) {
            oldLenDesc.value = JS::Any(index + 1);
            this->InternalObject::defineOwnProperty(u"length", oldLenDesc.value(), false);
        }
        return true;
    }
    return this->InternalObject::defineOwnProperty(key, attribute, is_throw);
...
JS::Any Array::toString(const JS::Any& thisArg, const JS::Any& args) {
    std::shared_ptr<JS::InternalObject> array = JS::CONVERT::ToObject(args[u"0"]);
    JS::Any func = array->get(u"join");
    if (!JS::COMPARE::Type(func, JS::OBJECT) ||
        !std::get<std::shared_ptr<JS::InternalObject>>(func.getValue())->isCallable()) {
...
JS::Any Array::toLocaleString(const JS::Any& thisArg, const JS::Any& args) {
    ...
    std::u16string R;
    if (JS::COMPARE::Type(firstElement, JS::UNDEFINED) ||
        JS::COMPARE::Type(firstElement, JS::NULL_TYPE)) {
        R = u"";
    } else {
    ...
    while (k < len) {
        ...
        if (JS::COMPARE::Type(nextElement, JS::UNDEFINED) ||
            JS::COMPARE::Type(nextElement, JS::NULL_TYPE)) {
            R = u"";
        } else {
...
JS::Any Array::join(const JS::Any& thisArg, const JS::Any& args) {
    ...
    std::u16string R = JS::COMPARE::Type(element0, JS::UNDEFINED) ||
                       JS::COMPARE::Type(element0, JS::NULL_TYPE)
                           ? u""
    ...
    while (k < len) {
        ...
        std::u16string next = JS::COMPARE::Type(element, JS::UNDEFINED) ||
                              JS::COMPARE::Type(element, JS::NULL_TYPE)
                                  ? u""
...
JS::Any Array::slice(const JS::Any& thisArg, const JS::Any& args) {
    ...
    uint32_t k = (relativeStart < 0)
                     ? std::max(static_cast<int32_t>(len + relativeStart), 0)
    ...
    uint32_t final = (relativeEnd < 0)
                         ? std::max(static_cast<int32_t>(len + relativeEnd), 0)
...
JS::Any Array::sort(const JS::Any& thisArg, const JS::Any& args) {
    ...
    if (JS::COMPARE::Type(args[u"0"], JS::UNDEFINED)) {
        ...
        std::ranges::sort(elements,
                          [&compareFn](const std::pair<std::u16string, JS::Any>& a,
                                       const std::pair<std::u16string, JS::Any>& b) {
                              return compareFn(a.second, b.second) < 0;
                          });
    } else if (JS::COMPARE::Type(args[u"0"], JS::FUNCTION)) {
        if (!std::get<std::shared_ptr<JS::InternalObject>>(args[u"0"].getValue())->isCallable()) {
...
JS::Any Array::splice(const JS::Any& thisArg, const JS::Any& args) {
    ...
    uint32_t len = JS::CONVERT::ToUint32(O->get(u"length"));
    int relativeStart = JS::CONVERT::ToInteger(args[u"0"]);
    uint32_t actualStart = (relativeStart < 0)
                               ? std::max(static_cast<int32_t>(len + relativeStart), 0)
                               : std::min(static_cast<uint32_t>(relativeStart), len);
    uint32_t actualDeleteCount = std::min(
        static_cast<uint32_t>(std::max(JS::CONVERT::ToInteger(args[u"1"]), 0)),
        len - actualStart);
    ...
    if (itemCount < actualDeleteCount) {
        ...
        uint32_t k = len - actualDeleteCount;
        while (k > actualStart) {
...
JS::Any Array::lastIndexOf(const JS::Any& thisArg, const JS::Any& args) {
    ...
    int n = JS::COMPARE::Type(args[u"0"], JS::UNDEFINED)
                ? static_cast<int>(len - 1)
                : JS::CONVERT::ToInteger(args[u"0"]);
    int k = (n >= 0)
                ? std::min(n, static_cast<int>(len - 1))
                : std::max(static_cast<int>(len + n), 0);
    while (k >= 0) {
        if (O->hasProperty(JS::CONVERT::ToString(k))) {
            JS::Any elementK = O->get(JS::CONVERT::ToString(k));
            if (elementK.strictEq(args[u"0"])) {
