...
JS::Any Function::get(const std::u16string& key) const {
    if (key == u"caller") {
        throw JS::Any(JS::InternalObject::create<JS::TypeError>(JS::Any("Cannot access 'caller' or 'arguments.callee' in strict mode")));
    }
...
bool Function::hasInstance(const JS::Any& value) const {
    ...
    if (!JS::COMPARE::Type(O, JS::OBJECT)) {
        throw JS::Any(JS::InternalObject::create<JS::TypeError>(JS::Any("Function.prototype[Symbol.hasInstance] called on non-object")));
    }
...
std::shared_ptr<JS::Function> JS::Function::getConstructor(std::shared_ptr<JS::InternalObject> instance) {
    ...
    constructor->properties = std::make_shared<JS::Properties>(JS::Properties{{u"length", JS::DataDescriptor{JS::Any(1), false, false, false}}, {u"name", JS::DataDescriptor{JS::Any(FUNCTION_CLASS_NAME), false, false, false}}, {u"prototype", JS::DataDescriptor(JS::Any(prototype), false, false, false)}});
    return constructor;
}
} // namespace JS