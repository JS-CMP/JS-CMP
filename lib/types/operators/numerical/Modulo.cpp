        ...
        switch (this->getValue().index()) {
            case NUMBER:
                switch (other.getValue().index()) {
                    case NUMBER:
                        return JS::Any(
                            std::fmod(std::get<double>(this->getValue()), std::get<double>(other.getValue())));
                    case STRING:
                        return JS::Any(std::fmod(std::get<double>(this->getValue()),
            ...
            case STRING:
                switch (other.getValue().index()) {
                    case NUMBER:
                        return JS::Any(std::fmod(JS::CONVERT::ToNumber(std::get<Rope>(this->getValue())),
            ...
            case BOOLEAN:
                switch (other.getValue().index()) {
                    case NUMBER:
                        return JS::Any(std::fmod(static_cast<double>(std::get<bool>(this->getValue())),
                                                 std::get<double>(other.getValue())));
                    case STRING:
                        return JS::Any(std::fmod(static_cast<double>(std::get<bool>(this->getValue())),
                                                 JS::CONVERT::ToNumber(std::get<Rope>(other.getValue()))));
...
JS::Any JS::Operator::operator%(int value) const {
    try {
        switch (this->getValue().index()) {
            ...
            case STRING:
                return JS::Any(
                    std::fmod(JS::CONVERT::ToNumber(std::get<Rope>(this->getValue())), static_cast<double>(value)));
            case BOOLEAN:
                return JS::Any(
                    std::fmod(static_cast<double>(std::get<bool>(this->getValue())), static_cast<double>(value)));
            case NULL_TYPE:
...
JS::Any JS::Operator::operator%(const char* value) const {
    try {
        switch (this->getValue().index()) {
            ...
            case STRING:
                return JS::Any(
                    std::fmod(JS::CONVERT::ToNumber(std::get<Rope>(this->getValue())), JS::CONVERT::ToNumber(value)));
            case BOOLEAN:
                return JS::Any(
                    std::fmod(static_cast<double>(std::get<bool>(this->getValue())), JS::CONVERT::ToNumber(value)));
            case NULL_TYPE:
...
Any operator%(int value, JS::Operator const& any) {
    try {
        switch (any.getValue().index()) {
            ...
            case STRING:
                return JS::Any(
                    std::fmod(static_cast<double>(value), JS::CONVERT::ToNumber(std::get<Rope>(any.getValue()))));
            case BOOLEAN:
                return JS::Any(
                    std::fmod(static_cast<double>(value), static_cast<double>(std::get<bool>(any.getValue()))));
            default:
                return JS::Any(std::numeric_limits<double>::quiet_NaN());
        }
    } catch (const std::invalid_argument&) {
...
Any operator%(const char* value, JS::Operator const& any) {
    try {
        switch (any.getValue().index()) {
            ...
            case STRING:
                return JS::Any(
                    std::fmod(JS::CONVERT::ToNumber(value), JS::CONVERT::ToNumber(std::get<Rope>(any.getValue()))));
            case BOOLEAN:
                return JS::Any(
                    std::fmod(JS::CONVERT::ToNumber(value), static_cast<double>(std::get<bool>(any.getValue()))));
            default:
                return JS::Any(std::numeric_limits<double>::quiet_NaN());
        }
    } catch (const std::invalid_argument&) {
        ...
        switch (any.getValue().index()) {
            ...
            case STRING:
                return JS::Any(std::fmod(std::numeric_limits<double>::quiet_NaN(),
                                         JS::CONVERT::ToNumber(std::get<Rope>(any.getValue()))));
            case BOOLEAN:
                return JS::Any(std::fmod(std::numeric_limits<double>::quiet_NaN(),
                                         static_cast<double>(std::get<bool>(any.getValue()))));
            default:
                return JS::Any(std::numeric_limits<double>::quiet_NaN());
        }
    } catch (const std::invalid_argument&) {
